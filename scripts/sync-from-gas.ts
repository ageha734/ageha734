...
interface CertRow { year: unknown; month: unknown; name_en: unknown; name_ja: unknown }
interface WorkRow { company_en: unknown; company_ja: unknown; start: unknown; end: unknown; role_en: unknown; role_ja: unknown; description_en: unknown; description_ja: unknown }
interface SkillRow { category: unknown; name: unknown; badge_url: unknown; icon_url: unknown }
interface ProjectRow { name: unknown; url: unknown; description_en: unknown; description_ja: unknown; stars_badge: unknown; forks_badge: unknown; issues_badge: unknown; prs_badge: unknown }
...
function buildSignedUrl(resourcePath: string): string {
    ...
    const signature = crypto.createHmac('sha256', SECRET as string).update(message).digest('hex')
...
async function main(): Promise<void> {
    ...
    for (const { path: rPath, key, normalize } of resources) {
        try {
            ...
            console.error(`Failed to sync ${key}: ${err instanceof Error ? err.message : String(err)}`)
        }
    }
